...
job_flow_overrides = {
    ...
    "Instances": {
        ...
        "Ec2SubnetId": "subnet-0b31a268c179f4606",
...
SPARK_STEPS_EXTRACTION = [
    {
        ...
        "HadoopJarStep": {
            ...
            "Args": [
                "s3://redfin-data-project1-yml/scripts/ingest.sh",
            ],
        },
    },
   ]
...
SPARK_STEPS_TRANSFORMATION = [
    {
        ...
        "HadoopJarStep": {
            ...
            "Args": ["spark-submit",
            "s3://redfin-data-project1-yml/scripts/transform_redfin_data.py",
            ],
        },
    },
   ]
...
with DAG('redfin_analytics_spark_job_dag',
        ...
        job_flow_id="{{ task_instance.xcom_pull(task_ids='tsk_create_emr_cluster', key='return_value') }}",
        step_id="{{ task_instance.xcom_pull(task_ids='tsk_add_extraction_step')[0] }}",# step_id return the list, we choose the first one
        target_states={"COMPLETED"},# Need to uppercase 
        timeout=3600,
        poke_interval=5,
        )
